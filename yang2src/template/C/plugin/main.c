/**
 * @file {{yang_name_short}}.c
 * @brief Example plugin for sysrepo datastore - {{yang_name_short}}.
 *
 * @copyright
...
/* retrieves & prints current {{yang_name_short}} configuration */
static void
retrieve_current_config(sr_session_ctx_t *session)
{
    ...
    log_msg("current {{yang_name_short}} configuration:");

    rc = sr_get_items(session, "/{{yang_name_short}}:*//*", &values, &count);
    if (SR_ERR_OK != rc) {
...
{
    log_msg("{{yang_name_short}} configuration has changed");
...
{
    log_msg("{{yang_name_short}} 'initialize' RPC called");

    if (input_cnt > 0) {
        log_fmt("{{yang_name_short}} tape content: %s", input[0].data.string_val);
    }
...
rpc_run_cb(const char *xpath, const sr_val_t *input, const size_t input_cnt,
        sr_val_t **output, size_t *output_cnt, void *private_ctx)
{
    log_msg("{{yang_name_short}} 'run' RPC called");
...
sr_plugin_init_cb(sr_session_ctx_t *session, void **private_ctx)
{
    ...
    rc = sr_module_change_subscribe(session, "{{yang_name_short}}", true, module_change_cb, NULL, &subscription);
    if (SR_ERR_OK != rc) {
        goto error;
    }

    rc = sr_rpc_subscribe(session, "/{{yang_name_short}}:initialize", rpc_initialize_cb, NULL, &subscription);
    if (SR_ERR_OK != rc) {
        goto error;
    }

    rc = sr_rpc_subscribe(session, "/{{yang_name_short}}:run", rpc_run_cb, NULL, &subscription);
    if (SR_ERR_OK != rc) {
        goto error;
    }

    log_msg("{{yang_name_short}} plugin initialized successfully");
...
error:
    log_fmt("{{yang_name_short}} plugin initialization failed: %s", sr_strerror(rc));
...
sr_plugin_cleanup_cb(sr_session_ctx_t *session, void *private_ctx)
{
    ...
    log_msg("{{yang_name_short}} plugin cleanup finished");
}
